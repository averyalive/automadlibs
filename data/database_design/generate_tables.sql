...
-- create views for specific word types
CREATE OR REPLACE SQL SECURITY INVOKER VIEW nouns AS
SELECT spelling FROM words
WHERE type="Noun"
ORDER BY RAND();

CREATE OR REPLACE SQL SECURITY INVOKER VIEW verbs AS
SELECT spelling FROM words
WHERE type="Verb"
ORDER BY RAND();

CREATE OR REPLACE SQL SECURITY INVOKER VIEW adjectives AS
SELECT spelling FROM words
WHERE type="Adjective"
ORDER BY RAND();

CREATE OR REPLACE SQL SECURITY INVOKER VIEW adverbs AS
SELECT spelling FROM words
WHERE type="Adverb"
ORDER BY RAND();
